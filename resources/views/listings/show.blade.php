<x-app-layout>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8 bg-white rounded-lg shadow p-6">
            ...
            <!-- Sludinājuma info un apraksts augšā -->
            ...
            <!-- Mazā pirmā bilde zem apraksta, centrēta -->
            <div class="flex justify-center my-4">
                <div class="w-32 h-32 sm:w-40 sm:h-40 rounded-lg overflow-hidden shadow-lg">
                    <img src="{{ $listing->images->first() ? Storage::url($listing->images->first()->path) : Storage::url('stockcar.png') }}"
                         alt="{{ $listing->marka }} {{ $listing->modelis }}"
                         class="w-full h-full object-cover object-center">
                </div>
            </div>

            <!-- Vairāk bildes galerija -->
            ...
            @if(auth()->check() && (auth()->user()->id === $listing->user_id || auth()->user()->is_admin))
                <div class="mt-6 flex flex-col sm:flex-row sm:space-x-4 space-y-2 sm:space-y-0">
                    ...
                    <form action="{{ route('listings.destroy', $listing->id) }}" method="POST" 
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-6 py-3 bg-red-600 text-white font-semibold rounded-lg shadow hover:bg-red-700 transition">
                            Dzēst sludinājumu
                        </button>
                    </form>
                </div>
