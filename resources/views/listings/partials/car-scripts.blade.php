<script>
    ...
    function cloneCarData(data) {
        try {
            return JSON.parse(JSON.stringify(data ?? {}));
        } catch (error) {
            console.error('Failed to clone car data', error);

            return {};
        }
    }

    function resolveCarData(source) {
        if (! source) {
            return {};
        }

        if (typeof source === 'string') {
            if (carDataCache[source]) {
                return cloneCarData(carDataCache[source]);
            }
            ...
            try {
                ...
                if (parsed && typeof parsed === 'object') {
                    carDataCache[source] = parsed;

                    return cloneCarData(parsed);
                }
            } catch (error) {
                console.error('Failed to parse car data JSON', error);
        ...
        if (typeof source === 'object') {
            return cloneCarData(source);
        }

        return {};
    }

    function carSelection(carDataSource, initialBrand = '', initialModel = '') {
        return {
            carDataSource,
            carData: {},
            ...
            init() {
                this.carData = resolveCarData(this.carDataSource);
