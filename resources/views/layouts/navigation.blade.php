<nav
    x-data="{ open: false }"
    class="sticky top-0 z-40 border-b border-white/30 bg-white/70 backdrop-blur-xl transition dark:border-slate-800/60 dark:bg-slate-950/80"
>
    ...
    <div class="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-8">
            ...
            <div class="shrink-0">
                <a
                    ...
                    class="flex items-center gap-2 rounded-full bg-white/70 px-3 py-1.5 shadow-sm ring-1 ring-white/60 transition hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900/60 dark:ring-slate-700/60"
                >
        ...
        <div class="hidden items-center gap-4 sm:flex">
            <button
                ...
                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/60 bg-white/80 text-indigo-600 shadow-sm ring-1 ring-white/50 transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-indigo-300"
            >
            ...
            @auth
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/80 px-4 py-2 text-sm font-medium text-slate-600 shadow-sm ring-1 ring-white/50 transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-slate-200"
                        >
            ...
            @else
                <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-600 transition hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">
                    {{ __('Ieiet') }}
                </a>
                <a href="{{ route('register') }}" class="rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 transition hover:-translate-y-0.5 hover:bg-indigo-500">
                    {{ __('Reģistrēties') }}
                </a>
            @endauth
        </div>
        ...
        <div class="-me-2 flex items-center sm:hidden">
            <button @click="open = ! open"
                class="inline-flex items-center justify-center rounded-full border border-white/50 bg-white/80 p-2 text-slate-600 shadow-sm transition hover:bg-white dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-slate-200"
            >
    ...
    <!-- Mobile Menu -->
    <div :class="{'block': open, 'hidden': ! open}" class="hidden border-t border-white/40 bg-white/80 backdrop-blur sm:hidden dark:border-slate-800/60 dark:bg-slate-950/80">
        ...
        <!-- Mobile User Info -->
        <div class="border-t border-white/40 px-4 py-3 dark:border-slate-800/60">
            @auth
                ...
                <div class="mt-4 space-y-1">
                    ...
                    <button type="button" @click="$store.theme.toggle()"
                        class="flex w-full items-center justify-between rounded-xl bg-white/80 px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm transition hover:bg-white dark:bg-slate-900/70 dark:text-slate-200 dark:hover:bg-slate-900"
                    >
